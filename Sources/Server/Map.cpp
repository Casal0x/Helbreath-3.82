...
CMap::CMap(class CGame *pGame) : m_bIsSnowEnabled(false) {
  int i, ix, iy;
  ...
  m_iMaximumObject =
      1000; // Default max objects per map (can be overridden by config)
  ...
  m_iMaxNx = m_iMaxNy = m_iMaxAx = m_iMaxAy = m_iMaxEx = m_iMaxEy = m_iMaxMx =
      m_iMaxMy = m_iMaxPx = m_iMaxPy = 0;
  ...
  for (i = 0; i < DEF_MAXSTRIKEPOINTS; i++) {
    ...
    std::memset(m_stStrikePoint[i].cRelatedMapName, 0,
                sizeof(m_stStrikePoint[i].cRelatedMapName));
  }
  ...
  for (i = 0; i < DEF_MAXDYNAMICGATES; i++) {
    ...
    std::memset(m_stDynamicGateCoords[i].m_cDynamicGateMap, 0,
                sizeof(m_stDynamicGateCoords[i].m_cDynamicGateMap));
  }
}
...
*** *	return value		:: void
***
**  commentary			::	-	added check to see if owner is
*class 1 or if is greater than max clients 			**
**********************************************************************************************************************/
void CMap::GetOwner(short *pOwner, char *pOwnerClass, short sX, short sY) {
  class CTile *pTile;
...
/*********************************************************************************************************************
**  void CMap::GetDeadOwner(short * pOwner, char * pOwnerClass, short sX, short
*sY)									**
...
/*bool CMap::sub_4C0F20(short dX, short dY)
{
 class CTile * pTile;
        ...
        if ((dX < 14) || (dX >= m_sSizeX - 16) || (dY < 12) || (dY >= m_sSizeY -
14)) return false;
...
bool CMap::bGetIsTeleport(short dX, short dY) {
  ...
  if ((dX < 14) || (dX >= m_sSizeX - 16) || (dY < 12) || (dY >= m_sSizeY - 14))
    return false;

  pTile = (class CTile *)(m_pTile + dX + dY * m_sSizeX);

  if (pTile->m_bIsTeleport == false)
    return false;

  return true;
}

void CMap::ClearOwner(int iDebugCode, short sOwnerH, char cOwnerType, short sX,
                      short sY) {
  class CTile *pTile;
  ...
  //
  if ((pTile->m_sDeadOwner == sOwnerH) &&
      (pTile->m_cDeadOwnerClass == cOwnerType)) {
...
class CItem *CMap::pGetItem(short sX, short sY, short *pRemainItemID,
                            char *pRemainItemColor,
                            uint32_t *pRemainItemAttr) // v1.4 color
{
  class CTile *pTile;
  class CItem *pItem;
  int i;
...
bool CMap::_bDecodeMapDataFileContents() {
  ...
  while (token != 0) {
    ...
    if (cReadMode != 0) {
      ...
      if (memcmp(token, "MAPSIZEX", 8) == 0)
        cReadMode = 1;
      if (memcmp(token, "MAPSIZEY", 8) == 0)
        cReadMode = 2;
      if (memcmp(token, "TILESIZE", 8) == 0)
        cReadMode = 3;
    }
...
bool CMap::bSearchTeleportDest(int sX, int sY, char *pMapName, int *pDx,
                               int *pDy, char *pDir) {
  int i;

  for (i = 0; i < DEF_MAXTELEPORTLOC; i++)
    if ((m_pTeleportLoc[i] != 0) && (m_pTeleportLoc[i]->m_sSrcX == sX) &&
...
void CMap::SetDynamicObject(uint16_t wID, short sType, short sX, short sY,
...
bool CMap::bGetDynamicObject(short sX, short sY, short *pType,
                             uint32_t *pRegisterTime, int *pIndex) {
  class CTile *pTile;
  ...
  *pRegisterTime = pTile->m_dwDynamicObjectRegisterTime;
  if (pIndex != 0)
    *pIndex = pTile->m_wDynamicObjectID;
...
bool CMap::bGetIsFarm(short tX, short tY) {
  ...
  if ((tX < 14) || (tX >= m_sSizeX - 16) || (tY < 12) || (tY >= m_sSizeY - 14))
    return false;

  pTile = (class CTile *)(m_pTile + tX + tY * m_sSizeX);

  if (pTile->m_bIsFarm == false)
    return false;
  ...
  for (i = 0; i < DEF_MAXNMR; i++) {
    if ((m_rcNoAttackRect[i].top > 0)) {
      ...
      for (ix = m_rcNoAttackRect[i].left; ix <= m_rcNoAttackRect[i].right; ix++)
        for (iy = m_rcNoAttackRect[i].top; iy <= m_rcNoAttackRect[i].bottom;
             iy++) {
          pTile = (class CTile *)(m_pTile + ix + iy * m_sSizeX);
          pTile->m_iAttribute =
              pTile->m_iAttribute | 0x00000004; // 3��° ��Ʈ�� �����Ѵ�. ��������
        }
...
bool CMap::bAddCrusadeStructureInfo(char cType, short sX, short sY,
                                    char cSide) {
  int i;
...
bool CMap::bRemoveCrusadeStructureInfo(short sX, short sY) {
  ...
  for (i = 0; i < DEF_MAXCRUSADESTRUCTURES; i++)
    if ((m_stCrusadeStructureInfo[i].sX == sX) &&
        (m_stCrusadeStructureInfo[i].sY == sY)) {
      m_stCrusadeStructureInfo[i].cType = 0;
      m_stCrusadeStructureInfo[i].cSide = 0;
      m_stCrusadeStructureInfo[i].sX = 0;
