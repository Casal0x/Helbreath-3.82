...
PartyManager::PartyManager(class CGame *pGame) {
  ...
  for (i = 0; i < DEF_MAXPARTY; i++) {
    ...
    std::memset(m_stMemberNameList[i].m_cName, 0,
...
CNP_BREAKLOOP:;
  ...
  for (i = 1; i < DEF_MAXPARTY; i++)
    if (m_stMemberNameList[i].m_iPartyID == 0) {
      ...
      // testcode
      std::snprintf(G_cTxt, sizeof(G_cTxt), "New party(ID:%d Master:%s)",
                    iPartyID, pMasterName);
      PutLogList(G_cTxt);
...
bool PartyManager::bDeleteParty(int iPartyID) {
  ...
  *wp = iPartyID;
  cp += 2;
  m_pGame->PartyOperationResultHandler(
      cData); // m_pGateCore->SendMsgToAllGameServers(0, cData, 10, true);
...
bool PartyManager::bAddMember(int iPartyID, char *pMemberName) {
  ...
  if (m_iMemberNumList[iPartyID] >= DEF_MAXPARTYMEMBER)
    return false;
  ...
  for (i = 1; i < DEF_MAXPARTY; i++)
    if (m_stMemberNameList[i].m_iPartyID == 0) {
      ...
      // testcode
      std::snprintf(G_cTxt, sizeof(G_cTxt), "Add Member: PartyID:%d Name:%s",
                    iPartyID, pMemberName);
      PutLogList(G_cTxt);
      return true;
    }
...
bool PartyManager::bRemoveMember(int iPartyID, char *pMemberName) {
  ...
  for (i = 1; i < DEF_MAXPARTY; i++)
    if ((m_stMemberNameList[i].m_iPartyID == iPartyID) &&
      ...
      // testcode
      std::snprintf(G_cTxt, sizeof(G_cTxt), "Remove Member: PartyID:%d Name:%s",
                    iPartyID, pMemberName);
      PutLogList(G_cTxt);
      return true;
    }
...
bool PartyManager::bCheckPartyMember(int iGSCH, int iPartyID, char *pName) {
  ...
  *wp = iGSCH;
  ...
  memcpy(cp, pName, 10);
  cp += 10;
  m_pGame->PartyOperationResultHandler(cData); // iRet =
                                               // m_pGateCore->m_pClientList[iClientH]->m_pXSock->iSendMsg(cData,
                                               // 20);
...
void PartyManager::CheckMemberActivity() {
  ...
  for (i = 1; i < DEF_MAXPARTY; i++)
    if ((m_stMemberNameList[i].m_dwServerChangeTime != 0) &&
      ...
      *wp = (uint16_t)m_stMemberNameList[i].m_iPartyID;
      ...
      // ?? ??? ?????? ??? ?? ??? ????.
      m_pGame->PartyOperationResultHandler(
          cData); // m_pGateCore->SendMsgToAllGameServers(0, cData, 22, true);
