...
void Screen_Test::on_render()
{
	...
	else
	{
		...
		int y = HEADER_HEIGHT + (row++ - m_scrollOffset) * ROW_HEIGHT + SCREENY;
		TextLib::DrawText(GameFont::Default, LEFT_COLUMN_X + SCREENX, y, "Bitmap fonts not loaded - run through Loading screen first", TextLib::TextStyle::Color(255, 100, 100));
	}
	...
	else
	{
		...
		{
			int y = HEADER_HEIGHT + displayRow * ROW_HEIGHT + SCREENY;
			TextLib::DrawText(GameFont::Default, LEFT_COLUMN_X + SCREENX, y, "Scroll down for Alignment Showcase...", TextLib::TextStyle::Color(100, 200, 255));
		}
	}
...
void Screen_Test::RenderHeader()
{
	...
	TextLib::DrawText(GameFont::Default, RIGHT_COLUMN_X + SCREENX, SCREENY + 45, "NEW (TextLib::DrawText)", TextLib::TextStyle::Color(150, 255, 150));

	// Divider line (visual separator)
	int midX = SCREENX + 320;
	for (int y = HEADER_HEIGHT; y < 480; y += 2)
	{
		m_pGame->m_Renderer->PutPixel(midX, SCREENY + y, 80, 80, 80);
	}
}
                                ...
                                void (Screen_Test::*newFunc)(int x, int y))
{
	...
	(this->*newFunc)(RIGHT_COLUMN_X + SCREENX, baseY + 16);
}
...
void Screen_Test::RenderAlignmentShowcase()
{
	...
	constexpr int GRID_X = SCREENX + 50;
	constexpr int GRID_Y = SCREENY + 100;
